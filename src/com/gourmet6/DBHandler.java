...
/**
 * A class which handles the database, i.e. gives methods to perform requests,
 * add, update and delete data from the DB.
 * 
 * @author Group 6
 * @version 04.05.2013
 */
public class DBHandler
{
	...
	private static class DBHelper extends SQLiteOpenHelper
	{
		...
		private static final String DB_NAME = "gourmet12.sqlite"; // as
																	// predicted
																	// !
	 ...
	 * 
	 * @param context
	 *            of the activity it is called in
	 */
	 ...
	 *             if the DB cannot be accessed for reading
	 */
	 ...
	 * 
	 * @return a String ArrayList containing the names of all the known towns in
	 *         alphabetical order.
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public String[] getTowns() throws SQLiteException
	{
		...
		try
		{
			c = this.db.query(true, TABLE_RESTAURANT, new String[]
			{ TOWN }, null, null, null, null, TOWN, null);
	 ...
	 * 
	 * @return an ArrayList of Locations containing a latitude, longitude and
	 *         town name
	 * @throws SQLiteException
	 */
	...
	/**
	 * Returns an ArrayList containing all the distinct town names appearing in
	 * the DB table restaurant, sorted in alphabetically.
	 * 
	 * @param town
	 *            a town in which to search restaurants
	 * @return a String ArrayList containing all the town names; if town is
	 *         null, returns all the towns.
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public ArrayList<String> getAllResNames(String town) throws SQLiteException
	{
		...
		try
		{
			if (town != null)
			{
				c = this.db.query(TABLE_RESTAURANT, new String[]
				{ RES }, TOWN + "=?", new String[]
				{ town }, null, null, RES);
			}
			else
	 ...
	 * @throws SQLiteException
	 */
	public ArrayList<Location> getAllResNamesLocation(String town)
			throws SQLiteException
	{
		String provider = LocationManager.PASSIVE_PROVIDER;
	...
	public Restaurant getRestaurant(String name) throws SQLiteException
	{
		...
		try
		{
			...
			c = this.db.query(TABLE_RESTAURANT, new String[]
			{ CHAIN, DESCRIPTION, LAT, LONG, STREET, ZIP, TOWN, TEL, MAIL, WEB,
					RATING, VOTES, SEATS, AVAIL, PICTURES }, RES + "=?",
					new String[]
					{ name }, null, null, null);
			if (c.getCount() > 1)
			{
				Log.e("DBHandler",
						"Error : two or more retaurants seem to have the same name.");
			}
			c.moveToFirst();
			...
			retour = new Restaurant(name, chain, address, town, tel, web, mail,
					description, rating, votes, zip, seats, availableSeats,
					latitude, longitude, priceCat, pictures);

			// cuisine
			c = this.db.query(TABLE_CUISINE, new String[]
			{ TYPE }, RES + "=?", new String[]
			{ name }, null, null, TYPE);
			...
			// timetable
			c = this.db
					.query(TABLE_TIMETABLE,
							new String[]
							{ DAY_START, DAY_END, TIME_OPEN, TIME_CLOSE },
							RES + "=?",
							new String[]
							{ name },
							null,
							...
							"CASE "
									+ DAY_START
									+ " WHEN 'lundi' THEN 1 WHEN 'mardi' THEN 12 WHEN 'mercredi' THEN 3 "
			...
			while (c.moveToNext())
			{
				...
				TimeTable tt = new TimeTable(dayStart, dayEnd, timeOpen,
						timeClose);
				timetable.add(tt);
			}
	 ...
	 * 
	 * @param resName
	 *            the name of the restaurant, must not be null
	 * @param rating
	 *            the new rating, must be between 0 and 5
	 * @param votes
	 *            the new number of votes, must be > 0
	 * @return 1 if update was performed on 1 row as expected, any other value
	 *         means an error has occurred
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for writing
	 * @throws SQLException
	 *             if one of the arguments is illegal
	 */
	public long rateRestaurant(String resName, float rating, int votes)
			throws SQLiteException, SQLException
	{
		...
		else if (resName == null)
		{
			throw new SQLException(
					"Error : no restaurant name given for request");
		}
		...
		try
		{
			...
			nrRows = this.db.update(TABLE_RESTAURANT, insertValues, RES + "=?",
					new String[]
					{ resName });
			if (nrRows > 0)
			{
	 ...
	 *            the name of the restaurant, must not be null
	 * @param newAvail
	 *            the new number of available seats, must be >= 0
	 * @return 1 if update was performed on 1 row as expected, any other value
	 *         means an error has occurred
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for writing
	 ...
	 *             if one of the arguments is illegal
	 */
	public long updateAvail(String resName, short newAvail)
			throws SQLiteException, SQLException
	{
	...
	/**
	 * Returns an ArrayList of the dishes served in a restaurant, sorted by type
	 * (starter, main course, dessert, drinks) and then alphabetically.
	 * 
	 * @param resName
	 *            the name of the restaurant
	...
	public ArrayList<Dish> getDishes(String resName) throws SQLiteException
	{
		...
		try
		{
			c = db.query(
					TABLE_DISH,
					new String[]
					{ DISH, RES, TYPE, SUBTYPE, DESCRIPTION, INVENTORY, PRICE },
					RES + "=?",
					new String[]
					{ resName },
					null,
					...
					" CASE "
							+ TYPE
							+ " WHEN 'Entr�es' THEN 1 WHEN "
							+ "'Plats' THEN 2 WHEN 'Desserts' THEN 3 WHEN 'Boissons' THEN 4 END, "
							+ SUBTYPE);
			...
			while (c.moveToNext())
			{
				...
				ArrayList<String> allergens = this.searchForAllergens(resName,
						dishName);

				// new dish object
				Dish dish = new Dish(dishName, type, subtype, price, inventory,
	 ...
	 */
	public long setDishInventory(String resName, String dishName, int newInvent)
			throws SQLiteException, SQLException
	{
		if (resName == null)
		{
		...
		{
			this.db.beginTransaction();
			nrRows = this.db.update(TABLE_DISH, updateValues, RES + "=? AND "
					+ DISH + "=?", new String[]
			{ resName, dishName });
			if (nrRows > 0)
			{
	 ...
	 *            , must not be null
	 * @param tel
	 *            the client's phone, optional
	 * @return if -1, then error; otherwise the rowId of the newly inserted data
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for writing
	 * @throws SQLException
	 *             if one of the arguments id invalid or if the insert fails
	 * @throws SQLiteConstraintException
	 *             if constraints on the DB are not respected
	 */
	public Client addClient(String mail, String name, String password,
			String tel) throws SQLiteException, SQLException,
	...
	public Client getClient(String mail) throws SQLiteException
	{
		...
		{
			// table client
			c = this.db.query(TABLE_CLIENT, new String[]
			{ CLIENT, TEL }, MAIL + "=?", new String[]
			{ mail }, null, null, null);
			if (c.getCount() > 1)
			{
				Log.e("DBHandler",
						"Error : two or more clients seem to have the same mail.");
			}
			c.moveToFirst();
	 ...
	 * 
	 * @param clientMail
	 *            the client's identifier (mail address)
	 * @param password
	 *            the password the client has entered
	 * @return true or false, depending on whether the password is the same as
	 *         the one found in the DB
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public boolean checkPassword(String clientMail, String password)
			throws SQLiteException
	{
		this.openRead();
		Cursor c;
		...
		{
			c = this.db.query(TABLE_CLIENT, new String[]
			{ PASSWORD }, MAIL + "=?", new String[]
			{ clientMail }, null, null, null);
			int count = c.getCount();
			if (count == 1)
			{
				c.moveToFirst();
			...
			{
				Log.e("DBHandler",
						"Error : the access to the DB must have failed.");
			}
		}
		finally
		{
	 ...
	 * 
	 * @param oldMail
	 *            the client's old mail (used to identify him), must not be null
	 * @param newMail
	 *            the client's new mail, must not be null
	 * @return 1 if update was performed on 1 row as expected, any other value
	 *         means an error has occurred
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for writing
	 ...
	 *             if constraints on the DB are not respected
	 */
	public long changeMail(String oldMail, String newMail)
			throws SQLiteException, SQLException, SQLiteConstraintException
	{
		...
		{
			this.db.beginTransaction();
			nrRows = this.db.update(TABLE_CLIENT, insertValues, MAIL + "=?",
					new String[]
					{ oldMail });
			if (nrRows > 0)
			{
	 ...
	 * 
	 * @param mail
	 *            the mail which identifies the client, must not be null
	 * @param newName
	 *            the new name the client wants to have in the DB, must not be
	 *            null
	 * @return 1 if update was performed on 1 row as expected, any other value
	 *         means an error has occurred
	 * @throws SQLiteException
	 ...
	 *             if one of the arguments is invalid
	 */
	public long changePassword(String mail, String newPassword)
			throws SQLiteException, SQLException
	{
		// throws an exception if the mandatory information is not given
		if (mail == null)
	 ...
	 * 
	 * @param mail
	 *            the client's mail which identifies him, must not be null
	 * @param newTel
	 *            the phone number the client wants to have in the DB, may be
	 *            null
	 * @return 1 if update was performed on 1 row as expected, any other value
	 *         means an error has occurred
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for writing
	 * @throws SQLException
	 *             if the first argument is illegal
	 */
			...
			SQLException
	{
		...
		{
			this.db.beginTransaction();
			nrRows = db.update(TABLE_CLIENT, insertValues, MAIL + "=?",
					new String[]
					{ mail });
			if (nrRows > 0)
			{
	 ...
	 * @param mail
	 *            the client's mail
	 * @return an ArrayList containing all orders relative to a client. All
	 *         dishes are included (excepted their description and inventory).
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public ArrayList<Order> getClientOrders(String mail) throws SQLiteException
	{
		...
		try
		{
			...
			c = this.db.query(TABLE_ORDER_OVERVIEW, new String[]
			{ "_id" }, MAIL + "=?", new String[]
			{ mail }, null, null, null);
			int count = c.getCount();
			if (count == 0)
	 ...
	 * 
	 * @param order
	 *            the order to insert into the DB, must not be null
	 * @return if -1, then error; otherwise the rowId of the newly inserted data
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 * @throws SQLException
	 *             if order is null or if insert fails
	 */
	public long addOrder(Order order) throws SQLiteException, SQLException
	{
		...
		try
		{
			...
			try
			{
				rowIdOverview = this.db.insertOrThrow(TABLE_ORDER_OVERVIEW,
						null, insertOverview);
				...
				for (Dish d : order.getOrderDishes())
				{
					...
					this.db.insertOrThrow(TABLE_ORDER_DETAIL, null,
							insertDetail);
				}
				this.db.setTransactionSuccessful();
			}
	...
	/**
	 * Gets all the reservations a client has ever made that are known to the
	 * DB.
	 * 
	 * @param mail
	 *            the client's mail
	 * @return an ArrayList containing all the reservations relative to a
	 *         client. If a reservation holds an order, it is contained.
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public ArrayList<Reservation> getClientReservations(String mail)
			throws SQLiteException
	{
		this.openRead();
		Cursor c;
		...
		try
		{
			...
			c = this.db.query(TABLE_RESERVATION, new String[]
			{ RES, ORDER_NR, DATETIME, PEOPLE }, MAIL + "=?", new String[]
			{ mail }, null, null, DATETIME);
			int count = c.getCount();
			if (count == 0)
			{
				return null;
			...
			while (c.moveToNext())
			{
				...
				// new Reservation
				Reservation reserv = new Reservation(resName, datetime, seats,
						client, mail);
	 ...
	 * 
	 * @param reservation
	 *            the reservation to insert, must not be null
	 * @param orderNr
	 *            if > 0, the identifier of the order bound to the reservation,
	 *            otherwise indicates that no order has to be bound.
	 * @return if -1, then error; otherwise the rowId of the newly inserted data
	 * @throws SQLiteException
	 *             if DB cannot be accessed
	 * @throws SQLException
	 *             if reservation is null or if insert fails
	 */
	public long addReservation(Reservation reservation, long orderNr)
			throws SQLiteException, SQLException
	{
		// throws an exception if the mandatory information is not given
		if (reservation == null)
		...
		insertValues.put(DATETIME,
				TimeTable.parseDateInString(reservation.getReservationTime()));
		...
		try
		{
			...
			try
			{
				rowId = this.db.insertOrThrow(TABLE_RESERVATION, null,
						insertValues);
				this.db.setTransactionSuccessful();
			}
			finally
	...
	/**
	 * Gets the number of seats available in a restaurant on a given day between
	 * two given hours, based on the reservations and the total number of seats
	 * in the DB.
	 * 
	 * @param resName
	 *            the restaurant's name
	 * @param dtStart
	 *            a String representing the date and start hour, format must be
	 *            yyyy-mm-dd hh:mm
	 * @param dtEnd
	 *            a String representing the date and ending hour, format must be
	 *            yyyy-mm-dd hh:mm
	 * @return the number of seats available between the dtStart and dtEnd
	 * @throws SQLiteException
	 *             if the DB cannot be accessed for reading
	 */
	public int getAvailBetweenDateTime(String resName, String dtStart,
			String dtEnd) throws SQLiteException
	{
		this.openRead();
		Cursor c;
		int seats;
		try
		{
			...
			c = this.db
					.rawQuery(
							"SELECT ("
									...
									+ " AND rest."
									+ RES
									+ "=? AND strftime('%Y-%m-%d %H:%M', reserv."
									+ DATETIME
									+ ") "
									+ "BETWEEN strftime('%Y-%m-%d %H:%M', ?) AND strftime('%Y-%m-%d %H:%M', ?)",
							new String[]
							{ resName, dtStart, dtEnd });
			int count = c.getCount();
			if (count != 1)
			{
			...
			{
				c = this.db.query(TABLE_RESTAURANT, new String[]
				{ SEATS }, RES + "=?", new String[]
				{ resName }, null, null, null);
				c.moveToFirst();
				seats = c.getInt(0);
			}
		}
	 ...
	 * @param town
	 *            the town name
	 * @return the town's latitude (average value of the restaurants in this
	 *         town)
	 */
	private double getTownLat(String town)
	{
		Cursor c = this.db.rawQuery("SELECT avg(" + LAT + ") AS newLat FROM "
				+ TABLE_RESTAURANT + " WHERE " + TOWN + "=?", new String[]
		{ town });
		if (c.getCount() > 1)
		{
	...
	{
		Cursor c = this.db.rawQuery("SELECT avg(" + LONG + ") AS newLong FROM "
				+ TABLE_RESTAURANT + " WHERE " + TOWN + "=?", new String[]
		{ town });
		if (c.getCount() > 1)
		{
	...
	{
		Cursor c = this.db.query(TABLE_RESTAURANT, new String[]
		{ LAT }, RES + "=? ", new String[]
		{ res }, null, null, null);
		if (c.getCount() > 1)
		{
	...
	{
		Cursor c = this.db.rawQuery("SELECT avg(price) as " + PRICE_CAT
				+ " FROM " + TABLE_DISH + " d, " + TABLE_RESTAURANT
				+ " r WHERE d." + RES + "=r." + RES + " AND r." + RES + "=?",
				new String[]
				{ resName });
		if (c.getCount() > 1)
		{
			Log.e("DBHandler",
					"Error : two or more restaurants seem to have the same name.");
		}
		double priceCat = 0;
		while (c.moveToNext())
		{
			...
			throws SQLiteException
	{
		...
		Cursor c = this.db.query(TABLE_ALLERGEN, new String[]
		{ ALLERGEN }, RES + "=? AND " + DISH + "=?", new String[]
		{ resName, dishName }, null, null, ALLERGEN);
		int count = c.getCount();
		if (count == 0)
		{
			return null;
	...
	{
		// information held by the client table
		Cursor c = this.db.query(TABLE_CLIENT, new String[]
		{ CLIENT }, MAIL + "=?", new String[]
		{ mail }, null, null, null);
		if (c.getColumnCount() > 1)
		{
			Log.e("DBHandler",
					"Error : two or more client seem to have the same mail.");
		}
		c.moveToFirst();
		String name = c.getString(c.getColumnIndex(CLIENT));
	...
	/**
	 * @param orderNr
	 *            the identifier of the order in the DB
	...
	private Order getOrder(int orderNr) throws SQLiteException
	{
		...
		c = this.db.query(TABLE_ORDER_OVERVIEW, new String[]
		{ RES, MAIL }, "_id=?", new String[]
		{ Integer.toString(orderNr) }, null, null, null);
		if (c.getCount() > 1)
		{
			Log.e("DBHandler",
					"Error : two or more orders seem to have the same number.");
		}
		...
		c = this.db.query(TABLE_ORDER_DETAIL, new String[]
		{ DISH, QUANTITY }, ORDER_NR + "=?", new String[]
		{ Integer.toString(orderNr) }, null, null, null);
		...
		while (c.moveToNext())
		{
			...
			d = this.db.query(TABLE_DISH, new String[]
			{ TYPE, SUBTYPE, PRICE }, DISH + "=? AND " + RES + "=?",
					new String[]
					{ dishName, resName }, null, null, null);
			if (d.getCount() > 1)
			{
				Log.e("DBHandler",
						"Error : two or more dishes seem to have the same name and restaurant.");
			}
			...
			Dish dish = new Dish(dishName, type, subtype, price, 0, null,
					allergens);
