...
public class TimeTable
{
	...
	public TimeTable(String jourDebut, String jourFin,
			GregorianCalendar openTime, GregorianCalendar closingTime)
	{
	...
	public TimeTable(String jourDebut, String jourFin, String openTime,
			String closingTime)
	{
		this.setJourDebut(jourDebut);
		this.setJourFin(jourFin);
	...
	static public HashMap<String, String> translateWeek = new HashMap<String, String>(
			7);
	static
	{
	...
	static public HashMap<Integer, String> mapWeek = new HashMap<Integer, String>(
			7);
	static
	{
	 ...
	 ***********************/
	/**
	 * Formats a String containing a date and/or time into a GregorianCalendar
	 * object representing the date.
	 * 
	 * @param date
	 *            the date to parse, may be one of the following 3 formats :
	 *            dd/MM/yyyy hh:mm dd-MM-yyyy hh:mm hh:mm
	 * @return a GregorianCalendar representing the date originally contained in
	 *         the String. If the format is not respected, returns null.
	 */
	...
	{
		String year = temp.get(GregorianCalendar.YEAR) + "-";
		String month;
	...
	public String parseInString()
	{
		...
		{
			return translateWeek.get(this.getJourDebut()) + " :  "
		...
		{
			return translateWeek.get(this.getJourDebut()) + " - "
					+ translateWeek.get(this.getJourFin()) + " :  "
					+ parseHorairInString(this.getOpenTime()) + " - "
					+ parseHorairInString(this.getClosingTime());
		}
	}
	...
	public boolean isInTimeTable(GregorianCalendar date)
	{
		...
		while (weekMap.get(this.getJourDebut()) != dateDebut
				.get(GregorianCalendar.DAY_OF_WEEK))
		{
			dateDebut.set(GregorianCalendar.DAY_OF_WEEK,
					dateDebut.get(GregorianCalendar.DAY_OF_WEEK) - 1);
		}
		dateDebut.set(GregorianCalendar.HOUR_OF_DAY,
				this.getOpenTime().get(GregorianCalendar.HOUR_OF_DAY));
		dateDebut.set(GregorianCalendar.MINUTE,
				this.getOpenTime().get(GregorianCalendar.MINUTE));
		...
		GregorianCalendar dateFin = new GregorianCalendar(
				...
				date.get(GregorianCalendar.HOUR_OF_DAY),
				date.get(GregorianCalendar.MINUTE));
		while (weekMap.get(this.jourFin) != dateFin
				.get(GregorianCalendar.DAY_OF_WEEK))
		{
			dateFin.set(GregorianCalendar.DAY_OF_WEEK,
					dateFin.get(GregorianCalendar.DAY_OF_WEEK) - 1);
		}
		dateFin.set(GregorianCalendar.HOUR_OF_DAY,
				this.getClosingTime().get(GregorianCalendar.HOUR_OF_DAY));
		dateFin.set(GregorianCalendar.MINUTE,
				this.getClosingTime().get(GregorianCalendar.MINUTE));

		return (date.compareTo(dateDebut) > 0 || date.compareTo(dateDebut) == 0)
				&& checkLimit(dateDebut, dateFin, date) && checkTime(date);
	}
	...
	private boolean checkTime(GregorianCalendar date)
	{
		...
		{
			if (this.getOpenTime().get(GregorianCalendar.MINUTE) <= date
		...
		{
			if (date.get(GregorianCalendar.MINUTE) <= this.getClosingTime()
					.get(GregorianCalendar.MINUTE))
			{
				return true;
			}
		...
		else if (heureDebut == heureDate && heureDate == heureFin)
		{
			if (this.getOpenTime().get(GregorianCalendar.MINUTE) <= date
					.get(GregorianCalendar.MINUTE)
					&& date.get(GregorianCalendar.MINUTE) <= this
							.getClosingTime().get(GregorianCalendar.MINUTE))
			{
				return true;
			}
			else
	...
	public boolean checkLimit(GregorianCalendar dateDebut,
			GregorianCalendar dateFin, GregorianCalendar date)
	{
		...
		{
			int heureFin = this.getClosingTime().get(
					GregorianCalendar.HOUR_OF_DAY);
			int heureDebut = this.getOpenTime().get(
					GregorianCalendar.HOUR_OF_DAY);
			int heureDate = date.get(GregorianCalendar.HOUR_OF_DAY);
			if (date.get(GregorianCalendar.YEAR) == dateFin
					.get(GregorianCalendar.YEAR)
					&& date.get(GregorianCalendar.MONTH) == dateFin
							.get(GregorianCalendar.MONDAY)
					&& date.get(GregorianCalendar.DAY_OF_MONTH) == dateFin
